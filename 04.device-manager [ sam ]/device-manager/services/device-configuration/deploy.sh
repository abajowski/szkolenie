...
aws cloudformation package --template-file infrastructure.yaml --s3-bucket $2 --output-template-file temp/device-configuration-tmp.yaml
if [ $3 == "L" ]
then
  echo -e '\n## DEPLOYING CLOUDFORMATION SCRIPT\n'
  aws cloudformation deploy  --template-file temp/device-configuration-tmp.yaml --stack-name $1 --capabilities CAPABILITY_NAMED_IAM --parameter-overrides ProjectName=$1 Environment=$5 NewDeviceTopicArn=$6 --region=$4
fi